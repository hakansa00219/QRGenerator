...
namespace QR.Encoder
{
    public class QREncoder
    {
        ...
        private readonly string alphaNumericPattern = @"^[0-9A-Za-z $%*+-./]+$";

        private byte charSize = 0;
        private EncodingType encodingType;
        private Texture2D texture;
        public EncodingType GetEncoding() => encodingType;
        public QREncoder(ref Texture2D QRTexture, string data, byte charSize)
        {
        ...
        {
            return data.All(x => char.IsDigit(x)) && charSize <= characterSizeTable[EncodingType.Numeric];
        }
